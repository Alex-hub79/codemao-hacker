...
module.exports = {
  async main() {
    ...
    let {postID, times, interval, threads} = await prompt([
      {name: "postID", message: "输入贴号(示例: 394580)", type: "number"},
      {
        name: "times",
      ...
      {name: "interval", message: "间隔时间 单位: 毫秒 (间隔太短可能导致封禁 默认100)", type: "number", default: 100},
      {name: "threads", message: "异步线程数量(过多或过少都会影响速度请根据电脑以及网络配置决定, 默认为25)", type: "number", default: 25}
    ]);
    let count = 0;
    ...
    while (count < times) {
      ...
      console.log(chalk`{blue 当前浏览量: ${count}} {green 目标进度: ${(count / times * 100).toFixed(2)}%} {cyan.bold 速度: ${(count - lastCount)}个/秒} {red 预计剩余时间: ${((times - count) / (count - lastCount)).toFixed(1)}秒}`);
